...
// Один осмотр — чистая функция от диска и git: что просрочено и какие правки это лечат.
// Отдельной функцией, потому что --fix зовёт её по кругу (см. внизу).
function осмотр() {
const беды = [];
...
// ⚑ --fix ДОВОДИТ ДО НЕПОДВИЖНОЙ ТОЧКИ (#234, 16.09). Поднятый маркер ПРАВИТ страницу, а значит
// просрочивает встречный маркер на соседе: index.html и docs/index.html ссылаются друг на друга.
// Один проход печатал «подняты маркеры: 1», выходил нулём — и следующий запуск снова краснел; видел
// это только CI после пуша (PR #233). Теперь проходы крутятся, пока осмотр не станет чистым, с
// потолком: не сошлось за ПРОХОДОВ — это настоящее противоречие, и оно называется вслух.
const ПРОХОДОВ = 5;
let итог = осмотр();
if (!ЧИНИТЬ) {
  for (const b of итог.беды) console.log(b);
  if (итог.беды.length) console.log('поднять всё разом: node tools/page-markers.mjs --fix');
  process.exit(итог.беды.length ? 1 : 0);
}
let всего = 0, проход = 0;
while (итог.правки.length && проход < ПРОХОДОВ) {
  проход++;
  for (const b of итог.беды) console.log(b);
  применить(итог.правки);
  всего += итог.правки.length;
  итог = осмотр();
}
if (итог.беды.length) {
  for (const b of итог.беды) console.log(b);
  console.log(`маркеры не сошлись за ${проход} проход(ов): осталось ${итог.беды.length} — противоречие, чинить руками`);
  process.exit(1);
}
console.log(всего ? `подняты маркеры: ${всего} за ${проход} проход(а)` : 'маркеры свежие');
process.exit(0);
