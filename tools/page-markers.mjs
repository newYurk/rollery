...
// ── 3. маркеры скриптов игры: ОДИН НА ВСЕХ
// Поштучные маркеры выглядят экономнее, но у них есть дыра, которой нет у общего.
// Проверка «скрипты старше index.html» ловит забытый бамп, сравнивая ВРЕМЯ файла со
// временем страницы. Правишь audio.js, а поднимаешь маркер у state.js — index.html
// становится новее всех, и забытый audio.js проходит молча. С единым маркером такого
// хода нет: любая правка требует поднять то самое число, и свежим становится всё.
// 05.09 в игре стояло три значения разом: 222 у одиннадцати файлов, 224 у трёх, 225 у трёх.
{
  const p = 'play/index.html';
  const html = readFileSync(join(ROOT, p), 'utf8');
  const вер = [...new Set([...html.matchAll(/src="[^"?]+\?v=(\d+)"/g)].map((m) => m[1]))];
  const без = [...html.matchAll(/src="([^"?]+)"(?![^<]*\?v=)/g)].map((m) => m[1]);
  if (без.length) скажи(`${p}: без ?v= — ${без.join(', ')}`);
  if (вер.length > 1) {
    скажи(`${p}: маркеры скриптов разные (${вер.join(', ')}) — общий маркер надёжнее поштучных`);
    const цель = String(Math.max(...вер.map(Number)) + 1);
    правки.push({ файл: p, было: null, стало: null, свести: цель });
  }
}

// ── 4. маяк у точек входа
...
if (ЧИНИТЬ && правки.length) {
  for (const { файл, было, стало, свести } of правки) {
    const путь = join(ROOT, файл);
    const txt = readFileSync(путь, 'utf8');
    if (свести) {
      writeFileSync(путь, txt.replace(/(src="[^"?]+)\?v=\d+"/g, `$1?v=${свести}"`));
      console.log(`  ✎ ${файл}: маркеры скриптов сведены к ?v=${свести}`);
      continue;
    }
