...
# Каталожная площадь СЕКТОРА. Мутация ВЫЖИВАЛА до 06.09, и не из-за тавтологии, а из-за
# НЕПОКРЫТОЙ ВЕТКИ: ни одна фикстура не строила патч с cut='сектор' (огурец F02 — брусок 8×8),
# поэтому формулу erratum-015 не проверял никто. Расписка «закроется вместе с #205» погашена:
# ветку покрыл не новый fixture, а независимый оракул, который уже лежал рядом и ни с чем не
# сверялся — sampleSector считает ту же площадь СЕТКОЙ 320², catalogAreaMm2 — замкнутой
# формулой. Сетка 76,9826 против формулы 76,9744; мутация уводит отношение к 0,714 при
# пороге 1,005. Проверка — core-v2.test.mjs, «площадь сектора сеткой сходится с формулой».
