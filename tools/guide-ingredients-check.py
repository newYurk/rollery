"""Размеры и нарезка начинок в гиде против каталога.

⚠ ФОРМУЛУ СТОРОЖ СВЕРЯЛ, А ЧИСЛА, КОТОРЫМИ ЕЁ КОРМЯТ, — НЕТ. У гида есть собственная
таблица ING (wU/hU/cut) ВНЕ помеченного блока формы, и именно она была прежним дефектом:
«креветка кружком 10×8» в гиде против полукруга 10×5 в игре. Сверять формулу и не сверять
...
if not m:
    print('  ✗ в гиде не нашлось таблицы ING — сверка не выполнена')
    sys.exit(3)
...
for kind, body in re.findall(r'(\w+):\s*\{([^}]*)\}', m.group(1)):
    src = re.search(r'\n\s*' + kind + r':\s*\{([^}]*)\}', c)
    if not src:
        print(f'  ✗ {kind}: есть в гиде, нет в каталоге'); bad += 1; continue
    sb = src.group(1)
    for f in ('wU', 'hU'):
        a = re.search(f + r':\s*([\d.]+)', body)
        b = re.search(f + r':\s*([\d.]+)', sb)
        if a and b and abs(float(a.group(1)) - float(b.group(1))) > 1e-9:
            print(f'  ✗ {kind}.{f}: гид {a.group(1)} · каталог {b.group(1)}'); bad += 1
    a = re.search(r"cut:\s*'([^']+)'", body)
    b = re.search(r"cut:\s*'([^']+)'", sb)
    if a and b and a.group(1) != b.group(1):
        print(f'  ✗ {kind}.cut: гид «{a.group(1)}» · каталог «{b.group(1)}»'); bad += 1

if not bad:
    print('  ✓ размеры и нарезка начинок в гиде совпадают с каталогом')
