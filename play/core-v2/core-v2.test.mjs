...
// ── #209, ПУНКТ 3: ДВА ПУТИ К ОДНОМУ ЯДРУ ОТВЕЧАЮТ ОДИНАКОВО ────────────────
// `adapt` — путь игры под `?v2`, `runFixture` — путь фикстур и песочницы. До 06.09 первый
// не звал `assessWinding`, и четыре отказа (`sheet_too_short`, `wraps_beyond_two`,
// `chef_corridor`, `core_overflow`) в игре молчали, а в песочнице срабатывали.
//
// ⚠ ПРОВЕРЯЕТСЯ СОГЛАСИЕ ПУТЕЙ, А НЕ КОНКРЕТНЫЙ КОД. Прибивать здесь «лист 40 мм даёт
// sheet_too_short» значило бы завести второе описание правил `assessWinding`; при их правке
// пришлось бы править и это место. Требование другое и оно от кода не зависит: чем бы ядро
// ни ответило, оба пути обязаны ответить ОДНИМ И ТЕМ ЖЕ.
test('#209: adapt и runFixture дают один вердикт на одном рецепте', () => {
  const базовый = makeF01Recipe();
  // Покрыты все физические отказы, до которых можно дотянуться рецептом: `sheet_too_short`,
  // `wraps_beyond_two`, `chef_corridor`. Четвёртый — `core_overflow` — недостижим, см.
  // отдельный todo ниже (замечание ревью на PR #231: сначала покрыты были только два).
  const случаи = [
    ['целый', базовый],
    ['лист короче оборота', { ...базовый, sheet: { ...базовый.sheet, lengthMm: 40 } }],
    ['лист вдвое длиннее', { ...базовый, sheet: { ...базовый.sheet, lengthMm: 420 } }],
    ['⌀ вне коридора повара', { ...базовый, sheet: { ...базовый.sheet, lengthMm: 140 } }],
    ['чужая база', { ...базовый, baseId: 'temaki' }],
  ];
  for (const [имя, r] of случаи) {
    const через_фикстуру = runFixture('cmp', r);
    const через_адаптер = adapt(r);
    assert.equal(через_адаптер.status, через_фикстуру.status,
      `${имя}: adapt даёт «${через_адаптер.status}», фикстура — «${через_фикстуру.status}»`);
    const a = (через_адаптер.diagnostics[0] || {}).code || null;
    const f = (через_фикстуру.diagnostics[0] || {}).code || null;
    assert.equal(a, f, `${имя}: коды диагностик разошлись — adapt «${a}», фикстура «${f}»`);
  }
});

// ── #209: ОТКАЗ `core_overflow` НЕДОСТИЖИМ ПО ПОСТРОЕНИЮ ────────────────────
// ⚠ `todo` НАМЕРЕННО, и это не «пока не сделали». Проверка красная, и красная она о КОДЕ.
//
// `assessWinding` отказывает, если AABB начинки выходит за коробку ядра. Но коробку считает
// `coreBoxMm` — как ОПИСАННЫЙ ПРЯМОУГОЛЬНИК ТЕХ ЖЕ САМЫХ патчей, которые потом и проверяет.
// Значит `ox = |cx| + w/2 − Wc/2` не может стать положительным никогда, а `oy` вдобавок ещё
// и уменьшен на толщину нори, которую `coreBoxesMm` добавляет к полувысоте.
//
// Перебор 735 конфигураций (три раскладки × семь ширин × семь высот × пять сдвигов):
// попаданий 0, максимум превышения ровно 0,000000 по x и −0,100000 по y.
//
// Это тот же сорт дефекта, что `Ravg` в #204: оракул выведен из проверяемого. Чинится не
// здесь — либо коробка должна приходить откуда-то независимо (паспорт базы, физический
// предел), либо отказ должен быть снят из реестра `erratum-023`. Вопрос контракта, и он
// записан в #209, а не решён молча.
test('#209: отказ core_overflow достижим хоть каким-нибудь рецептом', { todo: 'коробка ядра выведена из тех же патчей' }, () => {
  const bases = [makeF02Recipe(), makeF05Recipe()];
  let нашёлся = false, худшийOx = -1e9;
  for (const b of bases)
    for (const w of [2, 8, 18, 40])
      for (const h of [1, 6, 15, 30])
        for (const du of [-30, 0, 30]) {
          const r = { ...b, patches: b.patches.map((p, i) => ({ ...p, widthMm: w, heightMm: h, uMm: p.uMm + du * (i ? 1 : 0) })) };
          let wd;
          try { wd = buildWinding(r); } catch { continue; }
          if ((assessWinding(r, wd).diagnostics[0] || {}).code === 'core_overflow') нашёлся = true;
          for (const p of r.patches) {
            const pos = patchCorePos(r, p);
            худшийOx = Math.max(худшийOx, Math.abs(pos.x) + p.widthMm / 2 - wd.Wc / 2);
          }
        }
  assert.ok(нашёлся, `ни один рецепт не вызывает core_overflow; максимум превышения ${худшийOx}`);
});

// ── #205, ПУНКТ 2: ДЕФЕКТ ЗАПИСАН ИСПОЛНЯЕМЫМ, А НЕ ТОЛЬКО СЛОВАМИ ──────────
// ⚠ `todo` НАМЕРЕННО. Проверка сейчас КРАСНАЯ и это правда о коде, а не о ней: позиция куска
// в ядре не читает `uMm` вовсе. Замер — зонд едет 20 → 44 мм по листу при неподвижном соседе
// на 60, куски по листу НЕ перекрываются, и смещение в ядре ровно ноль на всём ходу.
//
// Шлюз она не валит по решению, а не по слабости: чинится это не правкой упаковки, а выбором
// между тремя утверждениями, любые два из которых исключают третье —
//   erratum-007: переупаковки нет вовсе, позиция чистая функция своих координат;
//   #186:        пустота внутри границы ядра < 10 %, ядро обтягивает пучок;
//   код:         пакует встык, поэтому выполняет второе и нарушает первое.
// Развилка описана в #205 и требует источников (собирает ли скрутка начинки в пучок), а не
// чтения кода. Как только её решат, этот тест либо позеленеет сам, либо будет удалён вместе
// с erratum-007 — и обе развязки лучше, чем дефект, записанный только прозой.
//
// Серия взята НЕПЕРЕКРЫВАЮЩАЯСЯ намеренно: у собственной серии F07 куски перекрываются на
// листе на каждом шаге (6…8 мм), и там непрерывность недостижима в принципе.
test('#205: центр в ядре обязан двигаться вместе с uMm', { todo: 'развилка erratum-007 против #186 не решена' }, () => {
  const EPS = 0.15;
  let prev = null;
  for (let u = 20; u <= 44; u += 2) {
    const r = makeF07Recipe(u);
    const [a2, b2] = r.patches;
    const след = (p) => [p.uMm - p.widthMm / 2, p.uMm + p.widthMm / 2];
    const [a0, a1] = след(a2); const [b0, b1] = след(b2);
    assert.ok(Math.min(a1, b1) - Math.max(a0, b0) <= 0, `u=${u}: куски перекрылись, серия негодна`);
    const pos = Object.fromEntries(r.patches.map((p) => [p.id, patchCorePos(r, p).x]));
    if (prev) {
      const d = Math.abs(pos['tamago-0'] - prev['tamago-0']);
      assert.ok(d > 0, `u=${u}: зонд проехал 2 мм по листу и не сдвинулся в ядре ни на сколько`);
      assert.ok(d <= 2 + EPS, `u=${u}: смещение ${d} больше хода 2 мм`);
    }
    prev = pos;
  }
});
