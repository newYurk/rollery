...
export function acceptF01(report, winding) {
  ...
  // ⚑ RAVG ПРИБИТ К ИЗМЕРЕННОМУ ПЕРИМЕТРУ (#204). Проверка `seam` выше сравнивает
  // |overlapMm − overlapArcRad·Ravg| — и это тождество, а не проверка: overlapArcRad
  // ОПРЕДЕЛЁН как overlapMm/Ravg, поэтому Ravg сокращается на обеих сторонах.
  // Невязка там 4,4e-6 при допуске 0,15 — не потому что модель верна, а потому что
  // выражение имеет вид a − (a/R)·R и равно нулю при ЛЮБОМ R.
  //
  // Доказано мутацией: `Ravg = (Rout − W/2) * 2` уводит угол шва вдвое (1,005 → 0,503)
  // и число двуслойных лучей вдвое (230 → 115), а ВСЕ фикстуры остаются зелёными.
  // В мутационном стенде она теперь стоит и обязана краснеть.
  //
  // Оракул независим не по названию, а по способу счёта: Ravg берётся МАКСИМУМОМ по
  // лучам (Rout = max rn), периметр — ИНТЕГРАЛОМ по тем же лучам (midArc). Для круглого
  // ролла они обязаны совпасть, и ошибка в любом из двух путей их разводит.
  //
  // ⚠ Когда ролл перестанет быть круглым (rp сегодня одинаков во всех лучах), максимум
  // станет больше периметра, делённого на τ, и эта строка покраснеет ЗАКОННО. Тогда её
  // надо не ослаблять, а переписать: угол шва обязан считаться по радиусу того слоя,
  // где лист реально лежит, а не по габаритному максимуму.
  const ravgFromArc = winding.noriPerimeter / TAU;
  push(Math.abs(ravgFromArc - winding.Ravg) <= EPS_LENGTH_MM
    ? ok('ravg')
    : fail('ravg', `периметр/τ ${ravgFromArc} против Ravg ${winding.Ravg}`));
