...
export function assessWinding(recipe, winding) {
  ...
  // ⚠ ЭТОТ ОТКАЗ НЕ СРАБАТЫВАЕТ НИКОГДА, И ЭТО ЗАМЕР, А НЕ ПОДОЗРЕНИЕ (#209).
  // `winding.Wc/Hc` считает `coreBoxMm` — как ОПИСАННЫЙ ПРЯМОУГОЛЬНИК ТЕХ ЖЕ патчей, которые
  // проверяются ниже. Значит `ox` не может стать положительным, а `oy` вдобавок уменьшен на
  // толщину нори, которую `coreBoxesMm` добавляет к полувысоте. Перебор 735 конфигураций:
  // попаданий 0, максимум превышения 0,000000 по x и −0,100000 по y.
  //
  // Тот же сорт, что `Ravg` в #204: оракул выведен из проверяемого. Не снято здесь потому,
  // что это вопрос контракта — либо коробка приходит откуда-то независимо, либо код уходит
  // из реестра `erratum-023`. Записано в #209; исполняемая запись — todo в core-v2.test.mjs.
