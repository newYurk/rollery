...
/**
 * ДОПУСК КАСАНИЯ СЛЕДОВ — ЭТО ПОГРЕШНОСТЬ ПРЕДСТАВЛЕНИЯ, А НЕ ДОПУСК ДЛИНЫ (#246, 16.09).
 * Бруски встык, поставленные дробными u, в double заходят друг на друга: uMm = u·L
 * округляется. Замер 16.09 (runs/continuation/task6-fixes/246/scale.cjs): 266 061 пара
 * «встык» на листах 105…609 мм, три способа набрать u — перекрытие не больше ОДНОГО ULP длины
 * листа, 1,14e-13 мм при 609 мм (самый длинный лист замера task4-nori: фруктовый ролл,
 * 4 витка; ULP величин из [512; 1024) = 2^−43 мм). На футомаки встык — 7,1e-15 мм.
 *
 * 1e-9 мм берётся с двух сторон:
 *   · снизу — ≈8800 ULP при 609 мм, запас на накопление в арифметике раскладки;
 *   · сверху — в тысячу раз мельче HASH_QUANTUM_MM (1e-6, hash.js) и на восемь порядков
 *     мельче EPS_LENGTH_MM: любое перекрытие, которое хеш отличает от нуля, остаётся отказом.
 * Абсолютный, а не относительный: длины в ядре — миллиметры, лист ограничен сверху.
 */
export const EPS_TOUCH_MM = 1e-9;
...
/**
 * ПОЛНЫЙ порядок для канонических массивов отчёта (ревью PR #223).
 *
 * Сортировка «по id» полным порядком НЕ является: у двух патчей id может совпасть. Приёмка
 * дубли не запрещает — замер 06.09: рецепт F05 с тремя патчами под одним id проходит как
 * `valid`, а перестановка массива даёт РАЗНЫЕ winding hash (2e5bbc0e74 против 4b1eb01e48).
 * То есть порядок массива протекал в отпечаток ровно там, где F05 обязан это ловить.
 *
 * Лечится не запретом дублей (это отдельный вопрос контракта — у erratum-012 нет для него
 * кода), а тем, что порядок делается полным: сначала id, потом сами значения. Два объекта,
 * не различимые ни по одному полю, взаимозаменяемы — и любой их порядок даёт один массив.
 *
 * ⚠ Дубли id остаются проблемой для ДИАГНОСТИК: `patch_material_overlap` кладёт в context
 * `patchIds: [a.id, b.id]`, и при совпадающих id сообщение перестаёт указывать на кусок.
 * Здесь это не чинится — записано в #205.
 */
export function byIdThenValue(keys) {
  return (a, b) => {
