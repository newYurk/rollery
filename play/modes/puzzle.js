...
//
// ⚠ Пазл живёт по СВОЕЙ физике листа: лист уровня — ДОЛЯ настоящего листа базы, и доля эта
// у старших уровней больше единицы (2,76 листа). Настоящий лист остаётся источником длины,
// но пазл берёт его с множителем — несведённое противоречие двух решений, issue #83.
//
// По решению владельца 29.08 режимы отложены: milestone «Оболочка и режимы».

// ---------------------------------------------------------------- пазл: повтори срез
// Уровень: n — начинок, share — ДОЛЯ ЛИСТА БАЗЫ (длина листа), pieces — сколько кусочков показано, wrap — обёрнутых нори,
// local — коротких начинок (видны не во всех кусочках), sheet — разрешён длинный ингредиент (лист омлета / крем-роза).
//
// ⚑ ЛИСТ УРОВНЯ — ДОЛЯ ЛИСТА БАЗЫ, А НЕ ВИТКИ (решение владельца 17.09, docs/design-core.md).
...
//
// Доля считается от b.L — длины настоящего листа базы (нори 10,5/21 см, лист омлета 42 см).
// Перевод нынешних витков в долю (полная таблица «было/стало» — в отчёте round4):
//   доля, которую нынешние витки дают у каждой базы
...
//   одной долей все базы не накрыть (у хосомаки и футомаки они расходятся в 1,81 раза), поэтому
//   взято среднее геометрическое двух опорных баз — хосомаки и футомаки: √(0,999·0,551) = 0,742,
//   √(2,136·1,183) = 1,589, √(3,698·2,053) = 2,755 → 0,74 · 1,59 · 2,76. Такая доля даёт наименьшее
...
//     доля 0,74 — хосомаки 104,9 → 104,9 · урамаки 113,0 → 113,0 · фруктовые 160,2 → 160,2 (все три
//       упираются в пол «не короче 2 витков», см. levelTurns) · тюмаки 108,7 → 118,4 · футомаки 115,6 → 155,4
//       · УЗУМАКИ 53,4 → 310,8;
//     доля 1,59 — 224,3 → 166,9 · 232,8 → 254,4 · 248,3 → 333,9 · 242,4 → 166,9 · фруктовые 348,7 → 333,9
//       · УЗУМАКИ 108,4 → 667,8;
//     доля 2,76 — 388,3 → 289,8 · 403,4 → 441,6 · 431,0 → 579,6 · 420,5 → 289,8 · 609,5 → 579,6
//       · УЗУМАКИ 182,2 → 1159,2.
const LEVELS = [
  { n: 1, share: 1.59, pieces: 1 },
  { n: 2, share: 1.59, pieces: 1 },
  { n: 3, share: 1.59, pieces: 1 },
  { n: 2, share: 2.76, pieces: 1, wrap: 1, rot: 1 },   // rot добавлен 02.09, лист 3→4 вит. 16.09, с 17.09 доля 1,59→2,76: без wrap и rot уровень — копия второго (#159, #168)
  { n: 3, share: 0.74, pieces: 1, shape: 'square' },
  { n: 3, share: 1.59, pieces: 3, local: 1 },
  { n: 4, share: 2.76, pieces: 1, shape: 'triangle' },
  { n: 3, share: 1.59, pieces: 6, local: 2 },
  { n: 3, share: 1.59, pieces: 6, rot: 1 },
  { n: 3, share: 2.76, pieces: 3, local: 1, wrap: 1, rot: 1 },   // то же, копия шестого (#159, #168)
  { n: 4, share: 2.76, pieces: 6, local: 2, wrap: 1 },
  { n: 4, share: 0.74, pieces: 3, local: 1, wrap: 1, sheet: 1, shape: 'square' },
  { n: 3, share: 1.59, pieces: 1, paint: 1, shape: 'triangle' },
  { n: 4, share: 1.59, pieces: 1, paint: 2, wrap: 1 },
  { n: 4, share: 1.59, pieces: 6, rot: 2, paint: 1 },
  { n: 5, share: 2.76, pieces: 6, local: 2, wrap: 2, sheet: 1, paint: 1 },
];
// ⚑ ДВА ПРАВИЛА О ДЛИНЕ ЛИСТА СХОДЯТСЯ ЗДЕСЬ.
// Первое — доля: лист уровня = share · b.L. Второе — «лист не короче 2 витков» (acceptTurns,
...
//   • на базах, где доля даёт меньше 2 витков (хосомаки 1,70, урамаки 1,63, фруктовые 1,97 при
//     доле 0,74), лист поднимается ровно до 2 витков — это и есть их нынешняя длина, уровень
...
const levelTurns = lv => {
  const b = B();
  if (lv.share == null) return turnsOf(lv.turns);   // пазл по ссылке: витки пришли с ней
  return acceptTurns(turnsFor(lv.share * b.L, b.T + b.w));
};
...
function puzzleStart(level, seed) {
  ...
  // Лист уровня считается ПОСЛЕ выбора базы: доля у уровня одна, длина у каждой базы своя
...
function levelTitle(lv, i) {
  ...
  // Лист в подписи — МИЛЛИМЕТРАМИ, а не витками и не долей: витки после 17.09 у каждой базы
  // свои, а доля игроку ничего не говорит. 105 мм он видит на циновке.
