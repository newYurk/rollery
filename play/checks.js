...
const REF = {
  ...
  uzumaki: { d: 46.6, dmax: 50.0, turns: 2.04, core: false },
  ...
  try {
    ...
    // спиралью, и что пустой лист не уезжает никуда.
    {
      const режимПри = (n, шаг) => {
        ...
        const mm = getModel();
        return { режим: mm.g.winding, метка: mm.needsSpiral };
      };
      ...
      // ⚑ НОРИ НЕ КРУТЯТ СПИРАЛЬЮ (#111, требование владельца). Размазанная раскладка на нори
      // остаётся КОЛЬЦОМ и помечается `needsSpiral` — «эта раскладка требует спирали, а обёртка
      // её не умеет». Что показывать игроку, владелец отложила; метка есть, выдумки нет.
      const нори = режимПри(7, 0.13);
      ok(нори.режим === 'ring', 'нори ушло в спираль — правило #111 прямо запрещает («нори — так не крутят»)');
      ok(нори.метка === true, 'перебор на нори не помечен needsSpiral — отказ смыкания молчит (#141)');
      // ⚑ А ТА ЖЕ РАСКЛАДКА НА ОМЛЕТЕ — СПИРАЛЬ. Омлет крутят витками (伊達巻, 渦巻き寿司),
      // поэтому переход разрешён именно ему. Это и проверяет, что переход жив, а не отключён.
      S.wrap = 'egg';
      const омлет = режимПри(7, 0.13);
      ok(омлет.режим === 'spiral', 'на омлете размазанная раскладка не стала спиралью — переход не работает (#141)');
