  ...
  try {
    ...
    for (const k of ['hoso', 'futo', 'ura']) {
      for (const hh of HANDS) {
        ...
        // ХОСОМАКИ НА ПРЕДЕЛЬНОЙ РУКЕ вышел за наш же потолок после того, как огурец переописан
        // полукругом по источникам (#108). Замер: 19,55 % → 20,69 %, и это НЕ объём — при равной
        // площади сечения число то же. Плоское дно шире растягивает бугор по витку.
        // Потолок 20 % — наша редакторская оценка «ролл всё же остаётся роллом», а не замер, и
        // подбирался он под старый каталог. Поднимать порог сразу после правки, из-за которой он
        // и упал, — так регрессия становится нормой. Поэтому проверка НЕ ослаблена, а переведена
        // в «известно» со ссылкой, ровно как у урамаки выше. Решение за владельцем.
        const roundKnown = k === 'hoso' && hh.name === 'воздух по максимуму';
        (roundKnown ? kn : say)(d.round <= ROUND_MAX_HAND,
          `${tag}: некруглость ${d.round.toFixed(1)} %, потолок ${ROUND_MAX_HAND}${roundKnown ? ' (issue #108)' : sfx}`);
