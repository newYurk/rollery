...
const REF = {
  ...
  // ⚑ ПЕРЕСНЯТО 31.08 после правки подворота (#113). Подворот стал решаться равенством
  // «остатка хватает ровно на оборот с нахлёстом», а не потолком 0,45·L — и это видно в
  // числах: обороты у всех баз сошлись к 1,09–1,15 (было 1,09–1,54), потому что лишний
  // виток исчез. ДИАМЕТРЫ ПОЧТИ НЕ СДВИНУЛИСЬ (28,9 → 29,1 · 53,9 → 54,7 · 33,0 → 33,0 ·
  // 52,9 → 53,3) — и так и должно быть: площадь сохраняется, менялась не масса, а укладка.
  // Отдельно стоит посмотреть на dmax − d: у хосомаки было 1,5 мм, стало 0,1; у футомаки
  // 2,8 → 0,1. Ролл стал КРУГЛЕЕ, потому что убралась ступенька от закопанного витка.
  hoso:  { d: 29.1, dmax: 29.2, turns: 1.15, core: true },
  futo:  { d: 54.7, dmax: 54.8, turns: 1.14, core: true },
  ura:   { d: 33.0, dmax: 33.1, turns: 1.09, core: true },
  fruit: { d: 53.3, dmax: 53.5, turns: 1.14, core: true },
};
