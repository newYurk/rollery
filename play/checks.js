...
function runChecks(detail) {
  ...
  try {
    ...
    // проверить. Уйдёт вместе с #152.
    {
      const форма = (n) => {
        S.base = 'futo'; S.shape = 'round'; S.hand = handOf();
        const л = [{ kind: 'tamago', u: 0.40, v: 0.5, z0: 0, z1: 0, phase: 1 }];
        for (let i = 1; i < n; i++) л.push({ kind: ['', 'cucumber', 'shiitake', 'kanpyo'][i], u: 0.40 + i * 0.07, v: 0.5, z0: 0, z1: 0, phase: 1 });
        S.lists.futo = л; clean(); touchModel(); layout();
        const mm = getModel(), ww = windFor(mm, 0.5), RR = mm.Rmax, N = 200;
        let x0 = 9, x1 = -9, y0 = 9, y1 = -9;
        for (let i = 0; i < N; i++) for (let j = 0; j < N; j++) {
          const xx = (i + 0.5) / N * 2 - 1, yy = (j + 0.5) / N * 2 - 1;
          if (Math.hypot(xx, yy) > 1) continue;
          const q = materialAt(mm, ww, 0.5, Math.hypot(xx, yy) * RR, Math.atan2(yy, xx));
          if (!q || q.cls !== 'patch' || q.mt.p.kind !== 'tamago') continue;
          x0 = Math.min(x0, xx); x1 = Math.max(x1, xx); y0 = Math.min(y0, yy); y1 = Math.max(y1, yy);
        }
        return x1 < x0 ? null : (y1 - y0) / Math.max(1e-6, x1 - x0);   // вытянутость куска
      };
      const один = форма(1), четыре = форма(4);
      if (один && четыре) {
        const рост = четыре / один;
        kn(рост < 1.35, `#152: тамаго меняет пропорции от числа соседей — ${один.toFixed(2)} один против ${четыре.toFixed(2)} с тремя, в ${рост.toFixed(1)} раза`);
      }
