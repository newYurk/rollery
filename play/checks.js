  ...
  try {
    ...
    for (const [w, h] of SIZES) {
      ...
      for (const б of ['hoso', 'futo', 'ura']) {
        ...
        ok(диск <= должно * 1.06,
           `${BASES[б].name}: ролл больше своего материала — диск ${диск.toFixed(1)} против ${должно.toFixed(1)} ед² ` +
           `(намазка ${намазка.toFixed(1)} + начинки ${начинки.toFixed(1)} + носитель ${(g.L * g.w).toFixed(1)}), ` +
           `лишку ${(100 * (диск - должно) / должно).toFixed(0)} %`);
      }
    ...
    for (const б of ['futo', 'uzumaki', 'hoso', 'ura']) {
      ...
      if (mm.g.winding === 'spiral') {
        ...
        ok(мм < 4.0, `${BASES[б].name}: лента прыгает на ${мм.toFixed(2)} мм — хуже достигнутого 3,47 (#153)`);
      }
