...
function layout() {
  ...
  if (mode === 'L') {
    ...
    // ⚑ РАЗМЕР ЧИПА ЗАДАЁТ ШИРИНА ЛЕНТЫ, А НЕ ЧИСЛО 44 (02.09, просьба владельца «просто всё
    // место занято»). Замер на iPhone 15 Pro: пять чипов по 44 — это лента 252 px в полосе 369,
    // то есть по 59 px пустоты с каждого бока. Чип растёт, пока лента не займёт полосу; потолок
    // 72 — выше он начинает спорить с листом за внимание. Размер известен ДО отбора, поэтому
    // ранжирование ниже считает высоту ряда по настоящему чипу, а не по минимальному.
    const размерПри = perR => Math.max(chipSize, Math.min(72, Math.floor((inner - (perR - 1) * chipGap) / perR)));
    ...
    const band = plP.band, rows = plP.rows, perRow = Math.ceil(n / rows);
    const размерЧипа = размерПри(perRow), рядЧипа = размерЧипа + 18;
    L.previewMode = showPrev ? (band ? 'band' : 'overlay') : 'none';
    // ⚑ ПАЛИТРА ПРИБИТА К НИЗУ, ЛИСТ С ЦИНОВКОЙ — ПО ЦЕНТРУ ОСТАВШЕГОСЯ (02.09).
    //
    // До этого лист прижимался к верху, а всё остальное ехало за ним. На футомаки выходило
    // ровно, а на урамаки — нет: у него лист 10,5 см против 21, на экране он выходит широким и
    // низким (199 px против 399), и под палитрой оставалось 210 px черноты. Прижать к верху и
    // прижать к низу — одинаково плохие правила, потому что оба отдают весь остаток одной
    // стороне.
    //
    // Правило такое: палитра — инструмент, её место у большого пальца, поэтому она стоит внизу
    // ВСЕГДА и не ездит от базы к базе. Лист же берёт столько, сколько позволяет его пропорция,
    // и центрируется в том, что осталось между шапкой и палитрой: остаток делится поровну, а не
    // сваливается в один угол. При коротком листе это читается как поле вокруг ролла, а не как
    // дыра под ним.
    const низЧипов = oy + ch - bottomPad;
    L.chips = { x: ox + 12, y: низЧипов - rows * рядЧипа, w: stripW, size: размерЧипа, rows, labels: true, perRow, pad };
    L.chipScroll = perRow * (размерЧипа + chipGap) - chipGap > inner;
    const верхЗоны = L.top + 26 + band, зонаH = (L.chips.y - 12) - верхЗоны;
    // ⚑ ЦИНОВКА ЛЕЖИТ ПОД ЛИСТОМ, А НЕ ПОД РАМКОЙ (02.09, #157). Здесь стояло `L.sheet.y + sh`,
    // где sh — высота РАМКИ, и при листе ниже рамки ручка отрывалась от него на половину
    // остатка, а весь стек под ней уезжал за край экрана. Ширина болела тем же: `sw + 36` брало
    // рамку, и на узком экране ручка вылезала за правый край. Обе величины берутся у листа.
    { const f = sheetFit(sw, Math.max(90, зонаH - (8 + handleH)), 'y');
      const блок = f.sh + 8 + handleH, y0 = верхЗоны + Math.max(0, (зонаH - блок) / 2);
