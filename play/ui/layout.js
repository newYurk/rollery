...
function layout() {
  ...
  if (mode === 'L') {
    ...
    const candsP = [];
    // Перебор идёт от ПОТОЛКА, а не от «сколько рядов нужно». Иначе вариант с ровными рядами
    // просто не рождался: пятнадцати чипов при девяти в ряду хватает двух рядов, и трёх никто
    // не предлагал. Лишние кандидаты безвредны — их отсеет либо пол листа, либо отбор ниже:
    // при равной видимости ранжирование само возьмёт тот, где лист больше.
    for (let r = Math.max(wantRows, ROWS_MAX); r >= 1; r--) { if (bandFull) candsP.push(fitP(bandFull, r)); candsP.push(fitP(0, r)); }
    // ⚑ РОВНЫЕ РЯДЫ (02.09, просьба владельца: «не хотим сделать равное количество иконок
    // в ряду?»). Пятнадцать чипов при двух рядах дают 8 + 7 — ступеньку, при трёх 5 + 5 + 5.
    // Само ранжирование этого не выберет никогда: оно судит по числу ВИДИМЫХ чипов, а потом
    // по площади листа, и при равной видимости два ряда всегда обгоняют три — лист больше.
    // Поэтому ровность — условие ОТБОРА, а не слагаемое веса: если среди вариантов, которые
    // по карману, есть делящие палитру нацело и показывающие её целиком, выбираем из них.
    // Если такого нет (узкий экран, где третий ряд пробивает пол листа), правило молча
    // уступает — ступенька лучше спрятанных начинок.
    const ровные = candsP.filter(c => c.rows > 0 && n % c.rows === 0 && c.vis >= n && c.alongU >= FLOOR);
    const plP = (ровные.length ? ровные : candsP).reduce(betterFit(FLOOR));
    const band = plP.band, rows = plP.rows, sh = plP.sh, perRow = Math.ceil(n / rows);
    L.previewMode = showPrev ? (band ? 'band' : 'overlay') : 'none';
    // ⚑ ЛИСТ ПРИЖАТ К ВЕРХУ БЮДЖЕТА, А НЕ ЦЕНТРИРОВАН (02.09). Центрирование `(sh − f.sh)/2`
    // стояло против «пустой циновки под листом» — но циновка теперь следует за ЛИСТОМ, а не за
    // рамкой (правка выше), и пустоты под ним не бывает. Осталась только пустота НАД ним:
    // остаток бюджета высоты, который лист не берёт, потому что держит свою пропорцию.
    // Замер на iPhone 15 Pro: бюджет выше листа на 90 px, половина из них — чёрная полоса
    // между панелью и циновкой, и владелец увидела её первой («просто всё место занято» —
    // а оно не занято). Прижимаем к верху, и весь остаток уходит вниз, где его забирают чипы.
    { const f = sheetFit(sw, sh, 'y');
      L.sheet = { x: ox + (cw - f.sw) / 2, y: L.top + 26 + band, w: f.sw, h: f.sh, uAxis: 'y', lenU: f.sh, lenV: f.sw }; }
    // ⚑ ЦИНОВКА ЛЕЖИТ ПОД ЛИСТОМ, А НЕ ПОД РАМКОЙ (02.09, #157). Здесь стояло `L.sheet.y + sh`,
    // где sh — высота РАМКИ, а L.sheet.y уже сдвинут на (sh − f.sh)/2 ради честной пропорции.
    // Когда лист уже рамки по высоте, ручка отрывалась от него на половину остатка, и весь
    // стек под ней — чипы и подписи — уезжал вниз на ту же величину, за край экрана. Ширина
    // болела тем же: `sw + 36` брало рамку, и на узком экране ручка вылезала за правый край,
    // а циновка выглядела сдвинутой вправо. Обе величины теперь берутся у самого листа.
    L.handle = { x: L.sheet.x - 18, y: L.sheet.y + L.sheet.h + 8, w: L.sheet.w + 36, h: handleH };
    // ⚑ ЧИПЫ ЗАНИМАЮТ ОСТАВШЕЕСЯ, А НЕ ДЕРЖАТ РАЗМЕР 44 (02.09, просьба владельца: «просто
    // всё место занято»). Замер на iPhone 15 Pro: лента из пяти чипов — 252 px в полосе 369,
    // то есть по 59 px пустоты с каждого бока, и ещё 58 px под лентой до низа экрана.
    //
    // Нижние 58 появились от правки циновки в этом же коммите: стек поехал за ЛИСТОМ, а не за
    // рамкой, место освободилось — и никто его не занял. Отдаём его чипам, а не листу: лист
    // свою пропорцию уже выбрал через sheetFit и лишнюю высоту не возьмёт (растянуть его —
    // значит соврать про форму), а палитре размер идёт впрок дважды — виднее иконка и крупнее
    // цель касания (44 — это нижняя норма, а не удобство).
    //
    // Растём по МЕНЬШЕМУ из трёх: сколько даёт ширина полосы, сколько даёт свободное дно, и
    // потолок 72 — выше чип начинает спорить с листом за внимание.
    const чиповВРяду = perRow;
    const свободноСнизу = (oy + ch - bottomPad) - (L.handle.y + handleH + 12 + rows * chipRow);
    const поШирине = Math.floor((inner - (чиповВРяду - 1) * chipGap) / чиповВРяду);
    const поВысоте = chipSize + Math.floor(Math.max(0, свободноСнизу) / rows);
    const размерЧипа = Math.max(chipSize, Math.min(поШирине, поВысоте, 72));
    const рядЧипа = размерЧипа + 18;
    L.chips = { x: ox + 12, y: L.handle.y + handleH + 12, w: stripW, size: размерЧипа, rows, labels: true, perRow, pad };
    // Действия над выбранным куском занимают СЛОТ ПОСЛЕДНЕГО РЯДА ЧИПОВ: пока кусок выбран,
    // ряд палитры прячется, а лист и остальные ряды не двигаются. Кнопка 44 px по центру
    // 62-пиксельного слота — чтобы подписи соседнего ряда не задевались.
    L.layBtn = { x: L.rowBtn.x, y: L.chips.y + (rows - 1) * рядЧипа + (рядЧипа - btnH) / 2, w: L.rowBtn.w, h: btnH, max: 3 };
    // Флаг для отрисовки: слот общий ТОЛЬКО в этой ветке. У планшета, десктопа и ландшафта
    // ряд кнопок стоит отдельно, и прятать там ряд палитры было бы потерей на ровном месте.
    L.chipsShareBtn = true;
    L.chipScroll = perRow * (размерЧипа + chipGap) - chipGap > inner;
