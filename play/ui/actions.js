...
function action(id) {
  switch (id) {
    ...
    case 'sheet': {
      ...
      // ⚑ ЛИСТ УРОВНЯ ПЕРЕСЧИТЫВАЕТСЯ ПОД НОВУЮ ОБЁРТКУ (17.09, #134/#242). Лист уровня — ДОЛЯ
      // ЛИСТА БАЗЫ, то есть ДЛИНА; витки — производная от неё и от шага витка (T + w). Обёртка
      // шаг меняет, и при тех же витках лист на циновке становится другой длины: замер 17.09 —
      // футомаки ур.1, подпись 334 мм, на циновке 349 (−4,4 %), то же у тюмаки, хосомаки и
      // урамаки. Без этой строки новое правило действовало до первого нажатия «●».
      ...
      // Пазл ПО ССЫЛКЕ (custom) не трогаем: там лист задан витками явно, а null — «лист базы».
      if (S.puzzle && S.puzzle.lv && S.puzzle.lv.share != null) S.turns = levelTurns(S.puzzle.lv);
