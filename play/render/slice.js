...
// Полилинии из трассировки. ⚠ ПУТИ НЕ ДОЛЖНЫ РВАТЬСЯ, и первая редакция их рвала.
//
// Замечание владельца 31.08: «я не ожидала, что эти пути могут быть прерывистыми». Она права,
// и права по существу: контур куска — замкнутая линия на плоском листе. Скрутка его
// деформирует, сминает, может сплющить почти в нить — но НЕ РАЗРЫВАЕТ. Разрыв в картинке
// означал бы, что кусок разорвало, а этого не бывает.
//
// Рвал алгоритм. Он соединял соседние лучи, пока ЧИСЛО границ на них совпадает, и начинал
// новую линию, как только счёт менялся. А счёт меняется постоянно и по невинной причине:
// на краю куска два пересечения луча сходятся в одно и исчезают. Каждый такой край давал
// разрыв — то есть штрихи стояли ровно там, где линия обязана быть целой.
//
// Теперь соседние границы сшиваются по БЛИЗОСТИ РАДИУСА, а не по номеру в списке. Граница
// продолжается той, что ближе всех по радиусу; если ближе порога никого нет — линия честно
// кончилась (кусок кончился), и это единственный законный разрыв.
const LINE_JOIN_DR = 0.06;          // насколько граница может сдвинуться за шаг луча, в долях R
function sliceLines(m, vSlice) {
  const к = traceSlice(m, vSlice);
  const живые = [];                  // тянущиеся сейчас пути: { точки, r }
  const готовые = [];
  for (let i = 0; i <= LINE_RAYS; i++) {
    const a = (i % LINE_RAYS) / LINE_RAYS * TAU, ряд = к[i % LINE_RAYS].slice();
    const занято = new Array(ряд.length).fill(false);
    const продолжены = [];
    const умершие = [];
    for (const путь of живые) {
      let best = -1, bd = LINE_JOIN_DR;
      ряд.forEach((rr, j) => { const d = Math.abs(rr - путь.r); if (!занято[j] && d < bd) { bd = d; best = j; } });
      if (best < 0) { умершие.push(путь); continue; }          // на этом луче продолжения нет
      занято[best] = true; путь.r = ряд[best];
      путь.точки.push([a, ряд[best]]);
      продолжены.push(путь);
    }
    // ⚠ ЗДЕСЬ КОНТУР НАЧИНКИ И ЗАМЫКАЕТСЯ. Кусок даёт на луче ДВЕ границы — вход и выход, —
    // и на краю куска они сходятся и пропадают обе разом. Если просто закрыть оба пути,
    // получатся две дуги вместо одного замкнутого контура: ровно те обрывки, которые
    // владелец и увидела. Поэтому умершие на одном шаге сшиваются между собой: два конца
    // одного контура, встретившиеся в точке заворота, — это он и есть, надо развернуть
    // второй и приклеить.
    while (умершие.length > 1) {
      const п = умершие.shift();
      let best = -1, bd = LINE_JOIN_DR * 2;
      умершие.forEach((q, k) => { const d = Math.abs(q.r - п.r); if (d < bd) { bd = d; best = k; } });
      if (best < 0) { готовые.push(п); continue; }
      const q = умершие.splice(best, 1)[0];
      готовые.push({ r: п.r, точки: п.точки.concat(q.точки.slice().reverse()) });
    }
    готовые.push(...умершие);
    живые.length = 0; живые.push(...продолжены);
    // границы, которым не нашлось продолжения, начинают свой путь
    ряд.forEach((rr, j) => { if (!занято[j]) живые.push({ r: rr, точки: [[a, rr]] }); });
  }
  готовые.push(...живые);
  // Замкнуть то, что обошло полный круг: начало и конец сошлись — соединяем, иначе видна щель.
  for (const п of готовые) {
    const п0 = п.точки[0], пN = п.точки[п.точки.length - 1];
    if (п.точки.length > LINE_RAYS * 0.9 && Math.abs(п0[1] - пN[1]) < LINE_JOIN_DR) п.точки.push(п0);
  }
  return готовые.map(п => п.точки).filter(p => p.length > 3);
}
