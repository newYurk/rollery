...
function drawPreviewArea(p) {
  ...
  if (pm === 'band') {
    ...
    if (pz) {
      ...
      // ⚠ ОКОШКО ЦЕЛИКОМ НА РИСЕ, А НЕ ПОВЕРХ ЧЕГО ПРИДЁТСЯ. Оно стояло от верхнего угла листа
      // и залезало на голую полосу нори (норисиро) и за правую кромку: получалась наклейка,
      // криво лежащая на границе двух материалов. Верх отсчитывается от НАЧАЛА РИСА, а не от
      // начала листа; правый край — с тем же полем, что и верхний.
      // Голая полоса у дальнего края — это (1 − spreadEnd) от высоты листа: там нори без риса,
      // и класть на неё окошко нельзя, оно перестаёт читаться как «что внутри».
      const fs = 88, поле = 10, дШир = fs + 2 * поле, поля = 8;
      const голая = (1 - B().spreadEnd) * s.h;
      const x = s.x + s.w - дШир / 2 - поля, y = s.y + голая + дШир / 2 + поля;
      drawMat(x - дШир / 2, y - дШир / 2, дШир, дШир, дШир / 2);
      ctx.save();
      ctx.beginPath(); ctx.arc(x, y, дШир / 2 - 1, 0, TAU); ctx.clip();
