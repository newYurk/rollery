...
function drawPreviewArea(p) {
  ...
  if (pm === 'band') {
    ...
    if (pz) {
      ...
      // ⚠ ПОДЛОЖКА ОБЯЗАНА ВМЕЩАТЬ ТЕНЬ. Поля были 7 px со всех сторон, а тень в пиксельном
      // режиме — это смещённый силуэт на (PIX, 2·PIX): снизу-справа она уезжала за кружок и
      // ложилась прямо на рис (владелец 31.08 прислала увеличенный кадр). Поле теперь считается
      // от самой тени, и снизу-справа его больше, чем сверху-слева, — как у настоящей тени.
      // ⚑ Кружок сдвинут внутрь на 14: он стоял в самом углу и налезал на скруглённый угол
      // листа и на тёмную полосу нори, читаясь наклейкой поверх, а не окошком внутрь.
      const fs = 88, тень = 2 * (typeof PIX === 'number' && PIX ? PIX : 0);
      const пВерх = 7, пНиз = 7 + тень;
      // Отступ считается от КРАЯ ПОДЛОЖКИ, а не от края картинки: иначе диск на пиксель-другой
      // вылезал за лист, и его скругление резалось рамкой.
      const дШир = fs + пВерх + пНиз;
      const x = s.x + s.w - fs / 2 - пНиз - 6, y = s.y + fs / 2 + пВерх + 6;
      drawMat(x - fs / 2 - пВерх, y - fs / 2 - пВерх, дШир, дШир, дШир / 2);
