...
function drawRolled() {
  ...
  if (!refusal) {
    // ЖИВОЙ СРЕЗ — ТОТ ЖЕ ЭЛЕМЕНТ, В ТОМ ЖЕ МЕСТЕ, ЧТО НА РАСКЛАДКЕ (#193).
    // Не «чтобы заполнить», а чтобы при переходе с листа на ролл игрок ничего не терял:
    // полоса стоит на своём `L.previewY`, который считается одинаково для обоих экранов.
    // Выключатель 👁 уважается сам собой — при `S.preview = false` режим станет 'none'.
    //
    // ⚠ ПОЛОСА СЧИТАЕТСЯ ЗДЕСЬ, А НЕ БЕРЁТСЯ У РАСКЛАДКИ, и вот почему. `drawPreviewArea`
    // умеет три режима, и два из них рисуют В ЛИСТ: 'overlay' — врезкой в угол `L.sheet`,
    // 'side' — в правую колонку `L.side`. На экране раскладки там лист; здесь листа нет, а
    // доска ролла шириной до 640 px занимает ровно эти места — врезка легла бы поверх ролла
    // (замер на ландшафте 852 × 393: режим 'side', колонка приходится на доску).
    //
    // Отдать только режим 'band' тоже нельзя: он включается не всегда. На футомаки в портрете
    // раскладка выбирает 'overlay' — чипов больше, полосе не хватает места НАД ЛИСТОМ. Но на
    // экране ролла листа нет и место есть: 279 px свободной высоты над доской при 393 × 852.
    // Поэтому здесь своя полоса, со своей меркой свободного места.
    drawRollPreview(R);
    drawRollFacts(R);
  }
  buttons = [];
...
// ── ЖИВОЙ СРЕЗ НАД ДОСКОЙ (#193) ────────────────────────────────────────────
// Тот же объект, что на экране раскладки, и по той же причине: при переходе с листа на ролл
// игрок не должен терять из виду то, ради чего всё делается. Размер и место — от СВОБОДНОГО
// МЕСТА над доской, а не от раскладочной сетки: на этом экране его больше и оно другое.
// ⚑ ГЕОМЕТРИЯ ОТДЕЛЬНО ОТ РИСОВАНИЯ — ЧТОБЫ СТОРОЖ ЧИТАЛ ЕЁ, А НЕ ПОВТОРЯЛ. Проверка
// «полоса не налезла на доску, подписи не налезли на кнопки» обязана считать те же числа,
// что и рисование; иначе это второе описание одной правды, и оно разъедется (в проекте это
// уже случалось не раз). Поэтому обе функции ниже возвращают геометрию, а рисуют по ней.
function rollPreviewGeom(R) {
  if (!S.preview && !S.puzzle) return null;
  const верх = L.top + 10, низ = L.roll.y - R - 36 - 12, место = низ - верх;
  if (место < 88) return null;                  // не влезает — молча ничего, а не внахлёст
  const fs = Math.min(116, место - 26, L.cw * 0.34);
  return { верх, низ, fs, cx: L.ox + L.cw / 2, cy: верх + место / 2 };
}
function drawRollPreview(R) {
  const g = rollPreviewGeom(R);
  if (!g) return;
  // В пазле полосу рисует раскладка: там она показывает ЦЕЛЬ, и повторять её логику здесь
  // значило бы завести второе описание одной картинки. Своя полоса — только для обычной игры.
...
// РАЗДУВАТЬ РОЛЛ НЕЛЬЗЯ — хосомаки настоящий 190 мм в длину и 29,7 в толщину, и пропорции
// здесь не украшение, а то, на чём стоит весь проект. Значит место просит СОДЕРЖАНИЯ.
//
// Что кладём и почему именно это. Экран нужен для одного решения — где резать; подсказка
// сверху так и говорит. Поэтому первой строкой идёт то, что из этого решения следует
// (сколько кусков и какой длины), а не то, что красивее. Риски резов на ролле были и раньше,
// подписи к ним не было — число рядом с ними и есть недостающая половина.
//
// Второй строкой — размеры самого ролла. Они уже посчитаны моделью и нигде игроку не
// показывались, хотя весь проект про то, что ролл настоящих пропорций.
//
// Третьей — что внутри. Срез сверху это ПОКАЗЫВАЕТ, но не НАЗЫВАЕТ; строка связывает
// раскладку с роллом словами.
//
// ⚠ Ниже зоны тапа. Рез ловится полосой `|y − L.roll.y| < R + 30` (`ui/actions.js`), доска
// кончается на `y + R + 36`. Текст начинается ниже неё, поэтому тап по подписи не режет ролл.
function rollFactLines(R) {
  ...
  // доска кончается на 280, первая строка ложится на 306, ряд кнопок начинается на 337 —
  // третья строка налезла бы на кнопки. Порядок строк не случаен: первой идёт то, ради чего
  // экран существует (где резать и сколько выйдет), последней — то, что можно и не увидеть.
  let y = L.roll.y + R + 36 + 26;
  const out = [];
  for (const л of все) {
    if (y + л.size / 2 > L.rowBtn.y - 10) break;
    out.push({ ...л, y });
    y += л.size + 10;
  }
  return out;
}
function drawRollFacts(R) {
  for (const л of rollFactLines(R)) {
    ctx.fillStyle = л.color; ctx.font = font(л.size); ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ctx.fillText(л.t, L.roll.x, л.y);
  }
}
// «1 кусок · 2 куска · 6 кусков» — иначе подпись читается как машинный шаблон.
function кусковСлово(n) {
  const d = n % 10, s = n % 100;
  if (s >= 11 && s <= 14) return 'кусков';
  if (d === 1) return 'кусок';
  if (d >= 2 && d <= 4) return 'куска';
  return 'кусков';
}
