...
function drawPreviewArea(p) {
  ...
  if (pm === 'band') {
    ...
    if (pz) {
      ...
      // ОКОШКО — В ПРАВОМ ВЕРХНЕМ УГЛУ ВСЕГО РАБОЧЕГО ЛИСТА, с равным отступом от границ.
      // Лист — это рис И нори: голая полоса у дальнего края такая же его часть, прятать от
      // неё окошко незачем. Пробовала прижать к началу риса — стало хуже: кружок повис
      // «где-то сверху справа», без внятной привязки к углу.
      // ПОЛЕ ШИРЕ ТЕНИ: тень в пиксельном режиме — смещённый силуэт на 2·PIX, и при узком
      // поле её срез читался хордой по кромке. Теперь обрезка ни разу не срабатывает и
      // остаётся страховкой, а не рисунком.
      const fs = 88, тень = 2 * (typeof PIX === 'number' && PIX ? PIX : 0);
      const поле = 10 + тень, дШир = fs + 2 * поле, поля = 10;
