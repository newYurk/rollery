...
function v2Refusal(snap) {
  ...
  const text =
    ...
    // ⚑ «риса» → «постели» (#255, 17.09): постель не у всех баз рисовая, а текст отказа базы
    // не знает вовсе — он собирается по коду. Нейтральное слово верно всегда.
    : d.code === 'patch_is_paint' ? 'Краска постели — не начинка ядра.'
