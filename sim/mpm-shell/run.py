#!/usr/bin/env python
"""mpm-shell: 2D MLS-MPM reference of rolling a sushi sheet (cross-section, plane strain).

Units: T = 1 rice thickness (~5 mm), rho_rice = 1, E_rice = 1, time unit = T / sqrt(E_rice/rho_rice).
All materials live on one MPM grid (Hu et al. 2018 MLS-MPM, mpm99 style). Nori = thin band of stiff
particles. The mat is a kinematic boundary: an arc of a circle that rolls without slipping along the table;
its radius is force-controlled (the "hand" presses with a prescribed pressure), see README.md.

CLI: python run.py --layout 1|2|3|4|5 --speed 1.0 --press 1.0 [--grid 320] [--particles 30000] [--out DIR]
"""
...
X_END_EXTRA = 3.0        # roll until tangent point passes the sheet end by this much
PHI_ROLL = 1.5 * math.pi # angular span of the mat during rolling (back + top + front)
...
R_INIT, R_MIN, R_MAX = 1.0, 0.8, 6.0
T_PRESS = 10.0           # duration of the pressing phase
GRAVITY = 0.01
MU_TABLE = 0.4
...
def build(nx, ny, n_part):
    ...
    @ti.kernel
    def substep(dt: float, xc: float, R: float, Rdot: float, vc: float, phi: float, shape: ti.i32, mu_mat: float):
        ...
        for I in ti.grouped(gm):
            if gm[I] > 0:
                ...
                px = X0 + I[0] * dx
                py = Y0 + I[1] * dx
                # --- mat (arc of a rolling circle, or square press)
                ddx = px - xc
                ddy = py - R
                if shape == 0:
                    ...
                    if dsd > -0.5 * dx and dsd < 2.5 * dx:
                        ...
                        if th <= phi:
                            ...
                            vb = ti.Vector([vc, Rdot]) + Rdot * ti.Vector([-sn, -cs]) + (vc - th * Rdot) * ti.Vector([-cs, sn])
                            vrel = vv - vb
                            vn = vrel.dot(n)
                            if vn < 0:
                                vt = vrel - vn * n
...
def compute_metrics(xs, vs, cls, Jp, nori_row, nori_col, info, layout, img, px, center, esc, extra):
    ...
    for i, f in enumerate(layout['fillings']):
        ...
        m = cls == c
        # identify this filling's particles: by class and by proximity to the initial rect (all same-kind fillings
        # in the presets are distinct kinds, so class suffices)
        if not m.any():
            continue
    ...
    met = dict(
        ...
        mat=extra['mat'], timing=extra['timing'],
    )
    return met
...
def main():
    ...
    ap.add_argument('--grid', type=int, default=320, help='total grid nodes ~ grid^2 (aspect follows the domain)')
    ap.add_argument('--particles', type=int, default=30000)
    ap.add_argument('--out', type=str, default=os.path.dirname(os.path.abspath(__file__)))
    ap.add_argument('--frames', type=int, default=12, help='number of debug snapshots (0 = none)')
    ...
    print(f'grid {nx}x{ny} dx={dx:.4f} particles={n} hp={info["hp"]:.4f} nori rows={info["nori_rows"]} '
          f'dt={dt:.5f} cmax={cmax:.2f} v_c={v_c} t_roll={t_roll:.1f} steps={n_steps}', flush=True)
    ...
    for step in range(n_steps):
        ...
        if not pressing:
            xc = X_MAT0 + v_c * t
            s_c = xc - X_MAT0
            # mat angular span: inextensible mat, capped; the ring closes over the last 3 T of travel
            close = min(1.0, max(0.0, (xc - (x_end - 3.0)) / 3.0))
            phi_max = PHI_ROLL + close * (2 * math.pi - PHI_ROLL)
            phi = min(phi_max, s_c / R)
            vc_now = v_c
            P_ref = P_ROLL_REF * args.press
            shp = 0
        else:
            xc = X_MAT0 + v_c * t_roll
            phi = 2 * math.pi
            vc_now = 0.0
            P_ref = P_PRESS_REF * args.press
            shp = shape
        S['substep'](dt, xc, R, Rdot, vc_now, phi, shp, MU_MAT)
        fnow = S['fn'][None]
        ...
        t += dt
        if step % 400 == 0:
            log.append(dict(t=round(t, 2), xc=round(xc, 3), R=round(R, 3), phi=round(phi, 3), F=round(F_f, 4), Ft=round(P_ref * (R * phi if shp == 0 else 8 * R), 4)))
        if args.frames and (step % snap_every == 0 or step == n_steps - 1):
            save_frame(S, cls, xc, R, phi, shp, os.path.join(frames_dir, f'f{step:07d}.png'), t, F_f)
        if step % 2000 == 0:
            el = time.time() - t0
            print(f'  step {step}/{n_steps} t={t:.1f} xc={xc:.2f} R={R:.3f} phi={phi:.2f} F={F_f:.3f} '
    ...
    # ---- outputs
    roll_m = np.ones(n, bool)
    center = (xs_f[:, 0].mean(), xs_f[:, 1].mean())
    # center the window on the centroid of everything (sheet is fully wound)
    ...
    extra = dict(layout=args.layout, speed=args.speed, press=args.press, R=R, window_T=args.window,
                 mat=dict(v_pull=v_c, P_roll=P_ROLL_REF * args.press, P_press=P_PRESS_REF * args.press, mu_mat=MU_MAT, mu_table=MU_TABLE,
                          press_shape=layout['press_shape'], phi_roll=PHI_ROLL, x_mat0=X_MAT0, x_end=x_end, t_roll=round(t_roll, 2), t_press=T_PRESS),
                 timing=dict(seconds=round(elapsed, 1), steps=n_steps, dt=round(dt, 6), grid=[nx, ny], dx=round(dx, 5), particles=n, hp=round(info['hp'], 5)))
    ...
    # final scatter with mat drawn
    save_frame(S, cls, xc, R, 2 * math.pi, shp, os.path.join(args.out, f'final_{tag}.png'), t, F_f, zoom=(center, args.window))
    print(json.dumps({k: v for k, v in met.items() if k not in ('controller_log', 'fillings')}, indent=1, default=_js))
    print(f'done in {elapsed:.1f}s')

def save_frame(S, cls, xc, R, phi, shp, path, t, F, zoom=None):
    import matplotlib
    matplotlib.use('Agg')
    ...
    th = np.linspace(0, phi, 200)
    if shp == 0:
        ax.plot(xc - R * np.sin(th), R - R * np.cos(th), 'r-', lw=1)
    else:
    ...
    ax.set_title(f't={t:.1f} xc={xc:.2f} R={R:.3f} phi={phi:.2f} F={F:.3f}', fontsize=8)
