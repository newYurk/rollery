...
CFL = 0.3
T_REL = 40.0            # phase E: mat removed, roll relaxes on the table (springback probe)
...
def main():
    ...
    while True:
        # ---------------- kinematic schedule ------------------------------------------------------
        grabbing = 1
        engaged = phase not in ('A', 'B', 'E_release')
        ...
        if phase == 'C' and step % 200 == 0:
            ...
            if ahead < C_EXIT_FRAC * R or xc >= x_end:
                ...
                np.savez_compressed(os.path.join(args.out, 'particles_' + tag + '_rollend.npz'), x=S['x'].to_numpy(), cls=cls, nori_row=nori_row, nori_col=nori_col, J=S['J'].to_numpy(), vol=vol); print('  [snap rollend] t=%.1f' % t, flush=True)
                xc = float(xnp[:, 0].mean())
        ...
        if phase == 'D_close' and t_phase >= T_CLOSE:
            ...
            np.savez_compressed(os.path.join(args.out, 'particles_' + tag + '_closed.npz'), x=S['x'].to_numpy(), cls=cls, nori_row=nori_row, nori_col=nori_col, J=S['J'].to_numpy(), vol=vol); print('  [snap closed] t=%.1f' % t, flush=True)
            if shape == 1:
        ...
        if phase == 'D_press' and t_phase >= T_PRESS and (abs(err_last) < 0.08 or t_phase >= t_press_max):
            np.savez_compressed(os.path.join(args.out, 'particles_' + tag + '_pressed.npz'), x=S['x'].to_numpy(), cls=cls, nori_row=nori_row, nori_col=nori_col, J=S['J'].to_numpy(), vol=vol)
            print('  [snap pressed] t=%.1f' % t, flush=True)
            phase = 'E_release'; t_phase = 0.0; phase_marks['E_release'] = t; R = R_MAX; Rdot = 0.0
        if phase == 'E_release' and t_phase >= T_REL:
            np.savez_compressed(os.path.join(args.out, 'particles_' + tag + '_released.npz'), x=S['x'].to_numpy(), cls=cls, nori_row=nori_row, nori_col=nori_col, J=S['J'].to_numpy(), vol=vol)
            print('  [snap released] t=%.1f' % t, flush=True)
            phase_marks['end'] = t
            if args.frames:
        ...
        step += 1
        if step > n_steps_max + int((60 + 2 * T_REL) / dt):
    ...
    extra = dict(layout=args.layout, speed=args.speed, press=args.press, tuck=tuck, R=R, window_T=args.window,
                 pitch=pitch_f, seed=args.seed, vol=vol, pred=pred, rice_map0=rice_map0,
