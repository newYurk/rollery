...
def predicted(mod, L):
    ...
    # ⚠ Первая редакция этого комментария писала «спрашиваю у самого прогона», хотя код уже
    # звал общий модуль, и соседняя фраза того же комментария говорила обратное. Поймано
    # вечерней сверкой 31.08. До того здесь стояло «same rule in both attempts» — тоже неправда:
    # редакций правила было три, все разные (#113).
