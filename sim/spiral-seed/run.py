...
def sample_spiral(layout, n_target):
    ...
    for i, f in enumerate(fl):
        ...
        u = min(u, L - layout['spread_tail'] - 0.6 - f['w'])     # never onto the glued tail
        sc = u + 0.5 * f['w']
        base_y = w_of(sc) + sp_of(sc) + f.get('y_off', 0.0)
        if f['stack'] and i > 0:
            base_y = rects[i - 1][1] + rects[i - 1][3]
        rects.append((u, base_y, f['w'], f['h'], f['round'], CLASS_OF_KIND[f['kind']]))
...
def spiral_from_particles(xs, cen, nori_row, nori_col, info):
    ...
    the roll centroid, unwrapped, is the winding angle: turns = |theta_end - theta_start| / 2pi -- an
    ...
    if len(tt) > 10 and tt[-1] - tt[0] > 2 * math.pi:
        ...
        pit = r1 - r0
    base = dict(turns_particles=round(float(turns_p), 3), tip_r_T=round(r_tip, 3),
