...
n2 = box(C2, 20, W2, 152,
         ...
         "Пишут 4 файла, читают 6. 176 обращений.",
         stroke=RED, bg=RED_BG)
...
n5 = box(C2, 554, W2, 168,
         ...
         "Объявлена в render/slice.js:373 и одной\n"
         "строкой делает три чужих дела:\n"
         "ключ модели · save() · dirty.\n"
         "Зовут из 6 файлов, 26 мест.",
         stroke=RED, bg=RED_BG)
...
s0 = box(C3, 20, W3, 128,
         "0 · Сторож из терминала — ПРЕДУСЛОВИЕ  #147\n\n"
         "Загрузчик ~50 строк, правок кода ноль.\n"
         "Весь checks.js идёт headless за 5,3 с.\n"
         "Без него ни один шаг ниже не доказуем.",
...
box(C3, 1150, W3, 228,
    ...
    "#146 топология кольца за имя — настоящий блокер\n"
    "        второй намотки, ревью его не назвало\n"
    "#142 спиральная база — ради чего всё\n"
...
box(C1, y + 480, W1 + 240, 232,
    ...
    "Карта нужна ради двух вопросов, и оба — не мои:\n\n"
    "1. Шаги 1–5 идут ДО #146/#142 или ПОСЛЕ?\n"
    "   (переезд ничего не чинит, но делает #142 дешевле)\n\n"
    "2. #74 (роль как свойство укладки) — до шага 2 или после?\n"
