...
#
# ⚠ ПОРЯДОК ВАЖЕН: сперва правка листов, ПОТОМ снятие слепка, и только затем сборка.
#   python3 docs/architecture/atlas-facts.py --snapshot   ← сначала
#   ./docs/architecture/build.sh                          ← потом
# Страница берёт штамп из atlas-facts.json на момент сборки. Собрал раньше слепка — и она
# называет состояние, которое уже не измерено (так вышло 05.09, поймало внешнее ревью).
#
