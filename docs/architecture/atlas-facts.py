...
def main():
    ...
    # ⚑ REF НЕ ДУБЛИРУЕТСЯ РУКАМИ НИ В ОДНОМ ЛИСТЕ (ревью на PR #211 и #212).
    #
    # Первая редакция сверяла ref в шапке листа 1 со слепком и краснела при расхождении.
    # Ревью показало, что это проверка ни о чём: ref из листа я тем же PR и убрала, регулярка
    # перестала находить что-либо, и шаг молча ничего не проверял. А если бы ref вернули и
    # он СОВПАДАЛ — проверка бы его приняла, и он протух бы к следующему слепку.
    #
    # Правило другое и проверяется прямо: коммит замера живёт ТОЛЬКО в atlas-facts.json и
    # печатается на странице. Ручная копия где угодно в листах — уже расхождение, потому
    # что отстанет по построению: правишь лист, снимаешь слепок, подпись мимо.
    # ⚠ ЛОВИТСЯ ФОРМА ШТАМПА, А НЕ ОДНА ФОРМУЛИРОВКА. Первая редакция требовала дословно
    # «снято на коммите» — и прошла мимо листа 2, где стояло «Слепок на коммите e6ae124»
    # (ref от 01.09, протух на четыре дня). Поймало ревью Greptile на PR #212.
    # Шаблон намеренно требует «на коммите»: упоминание ref в исторической прозе
    # («говорила 362ffdc при слепке eeca304») — это рассказ о прошлом, а не штамп.
    import re as _re2
    ШТАМП = _re2.compile(r"(?:снято|слепок)\s+на\s+коммите\s+([0-9a-f]{7,40})", _re2.IGNORECASE)
    for лист in sorted((ROOT / "docs" / "architecture").glob("0*.d2")):
        for m in ШТАМП.finditer(лист.read_text(encoding="utf-8")):
            stamp_drift.append((f'{лист.name}: ручная копия ref «{m.group(1)}»',
                                'ref живёт только в atlas-facts.json — убрать из листа'))
    ...
    if шаблон.exists():
        ...
        for m in ИТОГИ.finditer(текст):
            if (int(m.group(1)), int(m.group(2))) != (w, r):
                prose_drift.append((f"{m.group(1)} записей, {m.group(2)} чтений",
                                    f"{w} записей, {r} чтений"))
            # ⚑ И СУММА В СОСЕДНЕЙ ЯЧЕЙКЕ (ревью PR #230). Та же строка таблицы несёт итог
            # «записи + чтения» отдельным числом, и первая правка обновила прозу, а его — нет:
            # 233 и 215 при итоге 437 вместо 448. Проверка прозы этого не видела, потому что
            # смотрела только на два числа из трёх. Ловится тем же куском текста: числовая
            # ячейка, идущая сразу за фразой.
            хвост = текст[m.end():m.end() + 200]
            m2 = _re2.search(r'<td class="num">(\d+)</td>', хвост)
            if m2 and int(m2.group(1)) != w + r:
                prose_drift.append((f"итог по строке {m2.group(1)}",
                                    f"{w + r} (записи {w} + чтения {r})"))
